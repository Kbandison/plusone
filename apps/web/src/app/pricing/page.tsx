...
export default function PricingPage() {
  return (
    <main id="main" className="mx-auto w-full max-w-[612px] px-6 py-16 sm:py-24">
      ...
      <p className="mt-6 text-[15.3px] leading-[1.7] text-ink-2">{PRICING_INTRO}</p>
        ...
        {PLANS.map((plan) => (
          ...
          >
            <div className="flex flex-wrap items-baseline justify-between gap-3">
              <h2 className="text-[1.08rem]">{plan.label}</h2>
              <span className="text-[14.4px]">{formatPriceCents(plan.priceCents)}</span>
            </div>
            <p className="mt-1.5 text-[12.2px] text-ink-3">
      ...
      <section className="mt-14">
        <h2 className="text-[1.17rem]">What premium gives you</h2>
      ...
      <section className="mt-12">
        <h2 className="text-[1.17rem]">What it will never buy</h2>
        <p className="mt-4 text-[14.4px] leading-[1.7] text-ink-2">{PRICING_NEVER_NOTE}</p>
        <ul className="mt-5 flex flex-col gap-3">
          {PREMIUM_NEVER.map((item) => (
            <li
              key={item}
              className="border-l border-critical/40 pl-5 text-[14.4px] leading-[1.65] text-ink-2"
            >
              {item}
            </li>
          ))}
