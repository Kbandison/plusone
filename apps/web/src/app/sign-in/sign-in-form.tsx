...
export function SignInForm({ suggestedDialCode = "" }: { suggestedDialCode?: string }) {
  ...
  return (
    ...
    >
      ...
      <p className="text-[13px] leading-[1.6] text-ink-3">{C.smsConsent}</p>

      <Submit label={C.sendLabel} pending={sending} />

      <p className="text-[13.9px] text-ink-2">
        {C.newHere}{" "}
        <Link
...
function CodeForm({ sent, onChange }: { sent: SignInState; onChange: () => void }) {
  ...
  return (
    <div className="mt-10">
      ...
      <p className="mt-4 text-[15.4px] leading-[1.7] text-ink-2">{C.codeIntro}</p>
      ...
      <div className="mt-6 flex flex-wrap items-center gap-x-6 gap-y-3">
        ...
        >
          ...
          <button
            ...
            className="ease-brand text-[13.9px] text-ink-2 underline decoration-line-2 underline-offset-4 transition-colors duration-200 hover:text-ink disabled:opacity-55"
          >
        ...
        <button
          type="button"
          onClick={onChange}
          className="ease-brand text-[13.9px] text-ink-2 underline decoration-line-2 underline-offset-4 transition-colors duration-200 hover:text-ink"
        >
          {C.changeLabel}
        </button>
      </div>
      ...
      {resendState.error ? (
        <p role="alert" className="mt-3 text-[13.9px] text-critical">
          {resendState.error}
        </p>
      ) : null}
    </div>
