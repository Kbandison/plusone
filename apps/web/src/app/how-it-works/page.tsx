...
export default function HowItWorksPage() {
  return (
    <main id="main" className="mx-auto w-full max-w-[652.8px] px-6 py-16 sm:py-24">
      ...
      <p className="mt-6 text-[16.3px] leading-[1.7] text-ink-2">{HOW_IT_WORKS_INTRO}</p>

      <ol className="mt-16 flex flex-col gap-14">
        {HOW_IT_WORKS.map((step, index) => (
          <li key={step.id} id={step.id} className="scroll-mt-24">
            <p className="font-display text-[14.5px] text-accent">
            ...
            {step.quoted ? (
              <p className="mt-6 border-l-2 border-line-2 pl-5 text-[15.4px] leading-[1.7]">
                {step.quoted}
              </p>
            ) : null}
          </li>
