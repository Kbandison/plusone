...
export function SiteFooter({ current }: { current?: string }) {
  return (
    <footer className="mt-20 border-t border-line pt-8">
        ...
        <ul className="flex flex-wrap gap-x-7 gap-y-3">
          {LINKS.filter((link) => link.href !== current).map((link) => (
            <li key={link.href}>
              <Link
                href={link.href}
                className="ease-brand text-[14.5px] text-ink-2 underline decoration-line-2 underline-offset-4 transition-colors duration-200 hover:text-ink hover:decoration-accent"
              >
                {link.label}
              </Link>
            </li>
      ...
      <p className="mt-7 text-[13px] text-ink-3">{BRAND.name} is in build.</p>
    </footer>
  );
}
