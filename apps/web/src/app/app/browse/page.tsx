  ...
  return (
    <main id="main">
      ...
      <form className="mt-8 flex flex-wrap items-end gap-4" method="get">
        ...
        <label className="flex items-center gap-2.5 text-[13.9px]">
          <input
            type="checkbox"
            name="active"
            value="1"
      ...
      {rows.length === 0 ? (
        <p className="mt-10 text-[15.4px] text-ink-2">{C.browseEmpty}</p>
      ) : (
          ...
          {rows.map((row) => (
            <li key={row.id as string} className="rounded-xl border border-line-2 bg-surface p-5">
              <Link href={`/app/connect/${row.id as string}?source=browse`} className="block">
                ...
                <h2 className="mt-3 text-[1.152rem]">
                  {(row.display_name as string) ?? "Someone"}
                </h2>
                <p className="mt-1.5 text-[13.4px] text-ink-3">
                  {[row.age, row.distance_mi != null ? `${row.distance_mi} mi` : null]
                    .filter(Boolean)
                    .join(" · ")}
                </p>
                {row.intention ? (
                  <p className="mt-3 text-[13.9px] text-ink-2">
