...
export default async function RoomsPage() {
  ...
  return (
    <main id="main">
      ...
      {rooms.length === 0 ? (
        <p className="mt-8 text-[15.4px] text-ink-2">{C.roomsEmpty}</p>
      ) : (
        <ul className="mt-8 flex flex-col gap-3">
          {rooms.map((room) => (
            <li key={room.id as string}>
              ...
              >
                <h2 className="text-[1.103rem]">{room.title as string}</h2>
                {room.description ? (
                  <p className="mt-1.5 text-[13.9px] text-ink-2">{room.description as string}</p>
                ) : null}
              </Link>
            </li>
          ))}
