...
export default async function RoomPage({ params }: { params: Promise<{ roomId: string }> }) {
  ...
  return (
    <main id="main">
      ...
      {room.description ? (
        <p className="mt-3 text-[14.9px] leading-[1.7] text-ink-2">{room.description as string}</p>
      ) : null}

      {pinned?.title ? (
        <aside className="mt-6 rounded-xl border border-line-2 bg-surface-2 p-5">
          <h2 className="text-[1.009rem]">{pinned.title}</h2>
          {pinned.body ? <p className="mt-2 text-[13.9px] text-ink-2">{pinned.body}</p> : null}
        </aside>
      ) : null}
      ...
      <p className="mt-2 text-[13px] text-ink-3">{C.roomNoDmNote}</p>
      ...
      <ul className="mt-8 flex flex-col gap-4">
        ...
        {[...(messages ?? [])].reverse().map((message) => (
          <li key={message.id as string} className="rounded-lg border border-line px-5 py-4">
            ...
            <p id={`post-${message.id as string}`} className="text-[14.9px] leading-[1.65]">
