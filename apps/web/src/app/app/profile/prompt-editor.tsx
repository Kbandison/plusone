...
export function PromptEditor({ answers }: { answers: readonly ProfilePromptAnswer[] }) {
  ...
  return (
    <section className="mt-10 rounded-xl border border-line-2 bg-surface p-6">
      <h2 className="text-[1.152rem]">{C.promptsHeading}</h2>
      <p className="mt-3 text-[14.5px] leading-[1.7] text-ink-2">{C.promptsIntro}</p>

      {answers.length === 0 ? (
        <p className="mt-4 text-[13.9px] text-caution">{C.promptsEmpty}</p>
      ) : null}
      ...
      <form action={act} className="mt-6 flex flex-col gap-6">
        ...
        {rows.map((row, index) => (
          <div key={index} className="flex flex-col gap-2.5">
            <label className="text-[13.4px] text-ink-2" htmlFor={`prompt-${index}`}>
              {C.promptChoose}
            </label>
            <select
              id={`prompt-${index}`}
