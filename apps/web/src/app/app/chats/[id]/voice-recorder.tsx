    ...
    <div className="mt-4 flex flex-col gap-3">
      ...
      {state === "recording" ? (
        <div className="flex items-center gap-4">
          ...
          <span className="text-[13.6px] tabular-nums">{C.voiceRecording(seconds)}</span>
          <button
            type="button"
      ...
      {state === "review" || state === "sending" ? (
        <div className="flex flex-wrap items-center gap-3">
          ...
          <button
            ...
            disabled={state === "sending"}
            className="ease-brand text-[13px] text-ink-3 disabled:opacity-55 underline decoration-line-2 underline-offset-4 transition-colors duration-200 hover:text-ink"
          >
            {C.voiceDiscardLabel}
          </button>
        </div>
