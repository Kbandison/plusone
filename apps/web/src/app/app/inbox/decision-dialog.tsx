  ...
  return (
    <>
      <button
        ...
        className="ease-brand flex w-[99.5px] flex-col items-center gap-2 transition-opacity duration-200 hover:opacity-80"
      >
      ...
      <dialog
        ...
        className="ease-brand m-0 mt-auto w-full max-w-[616.3px] rounded-t-2xl border border-line-2 bg-surface p-6 text-ink backdrop:bg-black/45 sm:m-auto sm:rounded-2xl"
      >
        ...
        <figure className="mt-6 rounded-xl border border-line-2 bg-ground p-5">
          {decision.question ? (
            <figcaption className="text-[15.4px] tracking-[0.02em] text-ink-3 uppercase">
              {decision.question}
            </figcaption>
          ) : null}
          <blockquote id={`reply-${decision.id}`} className="mt-2 text-[20.2px] leading-[1.6]">
            {decision.reply}
          </blockquote>
        </figure>
...
function CloseIcon() {
  return (
    <svg viewBox="0 0 24 24" aria-hidden="true" focusable="false" className="size-[20.3px]">
      <path
