...
export default async function PremiumPage() {
  ...
  return (
    <main id="main">
      ...
      <p className="mt-5 text-[15.9px] leading-[1.7] text-ink-2">{C.premiumIntro}</p>
      ...
      {isPremium ? (
        <section className="mt-8 rounded-xl border border-accent bg-surface p-6">
          ...
          {!subscription && grantUntil ? (
            <p className="mt-2 text-[13.9px] text-ink-3">{C.premiumFromGrant}</p>
          ) : null}
      ...
      <section className="mt-14">
        <h2 className="text-[1.152rem]">{C.premiumIncludesHeading}</h2>
        <ul className="mt-5 flex flex-col gap-3">
          {PREMIUM_INCLUDES.map((item) => (
            <li
              key={item}
              className="border-l border-line-2 pl-5 text-[14.9px] leading-[1.65] text-ink-2"
            >
              {item}
            </li>
          ))}
      ...
      <section className="mt-12">
        <h2 className="text-[1.152rem]">{C.premiumNeverHeading}</h2>
        <p className="mt-4 text-[14.9px] leading-[1.7] text-ink-2">{C.premiumNeverNote}</p>
        <ul className="mt-5 flex flex-col gap-3">
          {PREMIUM_NEVER.map((item) => (
            <li
              key={item}
              className="border-l border-critical/40 pl-5 text-[14.9px] leading-[1.65] text-ink-2"
            >
              {item}
            </li>
          ))}
