...
export function BlockButton({
  ...
  if (state.message) {
    ...
    return (
      <span role="status" className="text-[13.4px] text-ink-3">
        {state.message}
      </span>
    );
  }
  ...
  return (
    <form action={act} className="inline">
      ...
      <button
        ...
        className="ease-brand text-[13.4px] text-ink-3 underline decoration-line-2 underline-offset-4 transition-colors duration-200 hover:text-critical disabled:opacity-55"
      >
        {C.blockLabel}
      </button>
    </form>
  ...
  return (
    <form action={act} className="inline">
      ...
      <button
        ...
        disabled={pending}
        aria-describedby={describedBy}
        className="ease-brand text-[13.4px] text-accent underline decoration-line-2 underline-offset-4 transition-colors duration-200 hover:decoration-accent disabled:opacity-55"
      >
        {C.unblockLabel}
      </button>
    </form>
...
export function ReportControl({
  ...
  const trigger = (
    <button
      ...
      aria-describedby={describedBy}
      aria-expanded={open}
      className="ease-brand text-[13.4px] text-ink-3 underline decoration-line-2 underline-offset-4 transition-colors duration-200 hover:text-ink"
    >
      {C.reportLabel}
    </button>
  );
  ...
  return (
    <>
      ...
      >
        <Heading className="text-[1.009rem]">{C.reportHeading}</Heading>
        <p className="text-[13.4px] leading-[1.6] text-ink-2">{C.reportIntro}</p>
        ...
        <fieldset className="flex flex-col gap-2.5">
          <legend className="mb-2 text-[13.4px] text-ink-2">{C.reportReasonLabel}</legend>
          {(Object.keys(REPORT_REASONS) as ReportReason[]).map((reason) => (
            <label
              key={reason}
              className="ease-brand flex cursor-pointer items-center min-h-tap gap-3 rounded-lg border border-line-2 bg-ground px-3.5 py-2.5 text-[13.9px] transition-colors duration-200 has-checked:border-accent"
            >
              <input
                type="radio"
                name="reason"
        ...
        {memberId ? (
          <label className="flex items-center gap-3 text-[13.9px]">
            <input type="checkbox" name="also_block" className="size-[17.3px] accent-accent" />
            {C.reportAlsoBlock}
          </label>
        ) : null}
