...
export function CrossCommunityToggle({ optIn }: { optIn: boolean }) {
  ...
  return (
    <section className="mt-10 rounded-xl border border-line-2 bg-surface p-6">
      <h2 className="text-[0.972rem]">{C.crossCommunityHeading}</h2>
      {/* §3.4, verbatim. Both sides have to say yes — the copy says so because
          the wall works that way. */}
      <p className="mt-4 text-[12.6px] leading-[1.7] text-ink-2">{COPY.crossCommunity.optIn}</p>
      ...
      <form action={act} className="mt-5 flex items-center gap-3">
        ...
        />
        <label htmlFor="cross_community" className="text-[12.2px]">
      ...
      {state.message ? (
        <p role="status" className="mt-3 text-[11.3px] text-positive">
          {state.message}
        </p>
      ) : null}
    </section>
  ...
  if (state.message) {
    return (
      <section className="mt-10 rounded-xl border border-line-2 bg-surface p-6">
        ...
        <p className="mt-4 text-[12.6px] leading-[1.7] text-ink-2">{state.message}</p>
      </section>
    );
  }
...
export function SignInEmail({ email, confirmed }: { email: string | null; confirmed: boolean }) {
  ...
  return (
    <section className="mt-10 rounded-xl border border-line-2 bg-surface p-6">
      <h2 className="text-[0.972rem]">{C.emailHeading}</h2>
      <p className="mt-4 text-[12.6px] leading-[1.7] text-ink-2">{C.emailBody}</p>
      <p className="mt-4 text-[12.2px] text-ink-2">{status}</p>

      <form action={act} className="mt-5 flex flex-wrap items-end gap-3">
        <div className="flex flex-col gap-2">
          <label htmlFor="sign_in_email" className="text-[12.2px]">
            {C.emailLabel}
          </label>
          <input
            id="sign_in_email"
      ...
      {state.error ? (
        <p id="sign_in_email-error" role="alert" className="mt-3 text-[11.3px] text-critical">
          {state.error}
        </p>
      ) : null}
