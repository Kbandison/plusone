...
export function QuizForm({ answered: given = {} }: { answered?: Record<string, string> }) {
  ...
  return (
    <form action={act} className="mt-10 flex flex-col gap-10">
      <p className="text-[13.4px] text-ink-3" role="status" aria-live="polite">
        {C.progress(answered, QUIZ_QUESTIONS.length)}
      </p>

      {QUIZ_QUESTIONS.map((question) => (
        <fieldset key={question.id} className="flex flex-col gap-3">
          <legend className="mb-3 text-[15.9px] leading-[1.5]">{question.question}</legend>
          {question.options.map((option) => (
            <label
              key={option.id}
              className="ease-brand flex cursor-pointer items-center min-h-tap gap-3.5 rounded-lg border border-line-control bg-surface px-4 py-3 text-[14.9px] transition-colors duration-200 has-checked:border-accent"
            >
              <input
                type="radio"
                name={question.id}
                ...
                className="size-[16.3px] shrink-0 accent-accent"
              />
              {option.label}
            </label>
          ))}
      ...
      {answered === 0 ? <p className="text-[13.4px] text-ink-3">{C.skipNudge}</p> : null}
    </form>
  );
}
