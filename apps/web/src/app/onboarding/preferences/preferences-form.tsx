...
function Choice({
  ...
  return (
    <fieldset className="flex flex-col gap-3">
      <legend className="mb-3 text-[14.5px]">{legend}</legend>
      <div className="flex flex-wrap gap-2.5">
        {Object.entries(options).map(([value, label]) => (
          <label
            key={value}
            className="ease-brand flex min-h-tap cursor-pointer items-center gap-2.5 rounded-lg border border-line-control bg-surface px-4 py-3 text-[14.9px] transition-colors duration-200 has-checked:border-accent"
          >
            <input
              type="radio"
              name={name}
              ...
              className="size-[16.3px] accent-accent"
            />
            {label}
          </label>
        ))}
        ...
        {optional ? (
          <label className="ease-brand flex min-h-tap cursor-pointer items-center gap-2.5 rounded-lg border border-line-control bg-surface px-4 py-3 text-[14.9px] text-ink-3 transition-colors duration-200 has-checked:border-accent has-checked:text-ink">
            <input
              type="radio"
              name={name}
              value=""
...
export function PreferencesForm({
  ...
  return (
    <form action={action} className="mt-10 flex flex-col gap-10">
      ...
      <fieldset className="flex flex-col gap-3">
        <legend className="mb-1 text-[14.5px]">{C.seekingLabel}</legend>
        <p className="mb-2 text-[13px] text-ink-3">{C.seekingHint}</p>
        <div className="flex flex-wrap gap-2.5">
      ...
      <div className="flex flex-col gap-8 border-t border-line pt-10">
        <div>
          ...
          <p className="mt-3 text-[13.9px] text-ink-3">{C.aboutHint}</p>
        </div>
      ...
      {!state.error && state.saved && savedMessage ? (
        <p role="status" className="text-[13.9px] text-ink-3">
          {savedMessage}
        </p>
      ) : null}
...
function AgeRange({ from, to }: { from: number | null; to: number | null }) {
  ...
  return (
    <fieldset className="flex flex-col gap-3">
      <legend className="mb-1 text-[14.5px]">{C.ageLabel}</legend>
      <p className="text-[13px] text-ink-3">{C.ageHint}</p>
