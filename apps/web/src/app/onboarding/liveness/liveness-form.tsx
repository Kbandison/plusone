...
export function LivenessForm() {
  ...
  return (
    <div className="mt-10 flex flex-col gap-6">
      ...
      {cancelled ? (
        <p role="status" className="text-[13.6px] text-ink-2">
          {C.cancelledBody}
        </p>
      ) : null}
      ...
      >
        <button
          ...
          className={buttonClass("primary", "w-full sm:w-auto sm:min-w-[189px] sm:self-start")}
        >
      ...
      {state.error && state.attemptsLeft > 0 ? (
        <p className="text-[12.6px] text-ink-3">{C.retriesLeft(state.attemptsLeft)}</p>
      ) : null}
...
function ReviewScreen({
  ...
  return (
    ...
    >
      ...
      <p className="mt-4 text-[14.4px] leading-[1.7] text-ink-2">{body}</p>
