    ...
    <>
      <form action={act} className="mt-8 flex flex-wrap gap-3">
        <input
          ...
          className="min-w-[194.4px] flex-1 rounded-lg border border-line-control bg-surface px-4 py-2.5 text-[16px] focus:border-accent"
        />
      ...
      <p role="status" className="mt-10 text-[13px] text-ink-2">
          ...
          {state.hits.map((hit) => (
            <li key={hit.user_id} className="rounded-xl border border-line-2 bg-surface p-6">
              <div className="flex flex-wrap items-baseline justify-between gap-3">
                <h2 className="text-[0.931rem]">{hit.display_name ?? "No name"}</h2>
                <span className="text-[11px] tracking-[0.04em] text-ink-3 uppercase">
                  {hit.verification_status}
                </span>
              </div>

              <p className="mt-2 text-[11.3px] text-ink-3">
