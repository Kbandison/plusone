  ...
  return (
    <main id="main">
      ...
      >
        ...
        <p className="mt-3 text-[13.9px] leading-[1.6] text-ink-2">
          {ghosted === 0
      ...
      {GROUPS.map((group) => (
        <section key={group.title} className="mt-10">
          <h2 className="text-[1.056rem] tracking-[0.04em] text-ink-3 uppercase">{group.title}</h2>
          <dl className="mt-4 grid grid-cols-2 gap-4 sm:grid-cols-3">
            {group.keys.map((key) => (
              <div key={key} className="rounded-lg border border-line-control bg-surface px-5 py-4">
                ...
                <dd className="mt-1.5 font-display text-[1.632rem] leading-none tabular-nums">
