"use client";

import { useActionState } from "react";

import { setConfig } from "./actions";
import { CONFIG_INITIAL } from "./state";
import { buttonClass } from "@/app/ui";

export function ConfigRow({ configKey, value }: { configKey: string; value: string }) {
  const [state, act, pending] = useActionState(setConfig, CONFIG_INITIAL);

  return (
    <li className="flex flex-wrap items-center gap-4 border-b border-line py-4 last:border-0">
      <code className="min-w-[230.4px] flex-1 text-[13.4px] text-ink-2">{configKey}</code>

      <form action={act} className="flex items-center gap-3">
        <input type="hidden" name="key" value={configKey} />
        <label className="sr-only" htmlFor={`v-${configKey}`}>
          {configKey}
        </label>
        <input
          id={`v-${configKey}`}
          name="value"
          type="number"
          step="any"
          defaultValue={value}
          className="w-[105.6px] rounded-lg border border-line-2 bg-ground px-3 py-2 text-[16px] tabular-nums focus:border-accent"
        />
        <button type="submit" disabled={pending} className={buttonClass("secondary")}>
          Save
        </button>
      </form>

      {state.error ? (
        <span role="alert" className="text-[13px] text-critical">
          {state.error}
        </span>
      ) : null}
      {state.message ? (
        <span role="status" className="text-[13px] text-positive">
          Saved
        </span>
      ) : null}
    </li>
  );
}
